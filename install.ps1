...
$Repo = if ($env:CLIOL_REPO) { $env:CLIOL_REPO.Trim() } else { "ezeprimo/cliol" }
$ApiBase = if ($env:CLIOL_API_BASE) { $env:CLIOL_API_BASE.TrimEnd('/') } else { "https://api.github.com" }
$RawBase = if ($env:CLIOL_RAW_BASE) { $env:CLIOL_RAW_BASE.TrimEnd('/') } else { "https://raw.githubusercontent.com" }
$RequestedVersionRaw = if ($env:CLIOL_VERSION) { $env:CLIOL_VERSION.Trim() } else { "latest" }
if (-not $RequestedVersionRaw) { $RequestedVersionRaw = "latest" }
$AssetName = "cliol-windows-amd64.exe"
$ChecksumsAssetName = "checksums.txt"
$InstallDir = Join-Path $env:LOCALAPPDATA "cliol\bin"
$TargetPath = Join-Path $InstallDir "cliol.exe"
$SmokePattern = if ($env:CLIOL_SMOKE_PATTERN) { $env:CLIOL_SMOKE_PATTERN } else { "cliol" }
$DisablePersistentPathUpdate = $env:CLIOL_DISABLE_PERSIST_PATH_UPDATE -eq "1"

function Normalize-RequestedVersion([string]$Version) {
    if ($Version -eq "latest") { return "latest" }; if ($Version.StartsWith("v")) { return $Version }; return "v$Version"
}
function Get-GitHubRelease([string]$Repository, [string]$RequestedTag, [string]$ApiBaseUrl) {
    $headers = @{ "Accept" = "application/vnd.github+json"; "User-Agent" = "cliol-installer" }
...
function Write-NextSteps([string]$ResolvedTag, [string]$Repository, [string]$RawBaseUrl, [string]$Reason = "") {
    $PackageVersion = $ResolvedTag.TrimStart("v"); $InstallScriptUrl = "$RawBaseUrl/$Repository/$ResolvedTag/install.ps1"
    if ($Reason) { Write-Warning "Binary install stopped: $Reason"; Write-Host ""; Write-Host "Fallback: pipx install --force cliol==$PackageVersion"; Write-Host "  python -m pip install --user cliol==$PackageVersion"; Write-Host "" }
    Write-Host "Update to latest:"; Write-Host "  Remove-Item Env:CLIOL_VERSION -ErrorAction SilentlyContinue"; Write-Host "  irm $InstallScriptUrl | iex"; Write-Host ""
    Write-Host "Uninstall:"; Write-Host "  irm $RawBaseUrl/$Repository/main/uninstall.ps1 | iex"
}
function Install-Atomically([string]$DownloadedPath, [string]$FinalPath) {
    ...
    if (Test-Path -LiteralPath $FinalPath) { $backup = "$FinalPath.bak"; [System.IO.File]::Replace($DownloadedPath, $FinalPath, $backup, $true); if (Test-Path -LiteralPath $backup) { Remove-Item -LiteralPath $backup -Force }; return }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("cliol-installer-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    if ($pathEntries -notcontains $InstallDir.TrimEnd('\\')) { $env:Path = if ($env:Path) { "$InstallDir;$env:Path" } else { $InstallDir } }
    Write-Host "Installed cliol $ResolvedTag to $TargetPath"
    if ($pathResult -eq "updated") { Write-Host "Added $InstallDir to user PATH. Open a new shell before running 'cliol'." }
    elseif ($pathResult -eq "already-present") { Write-Host "User PATH already contains $InstallDir." }
    elseif ($pathResult -eq "disabled") { Write-Host "Persistent PATH update disabled via CLIOL_DISABLE_PERSIST_PATH_UPDATE." }
    else { Write-Warning "Could not update user PATH: $pathResult"; Write-Host "Add this path manually: $InstallDir" }
